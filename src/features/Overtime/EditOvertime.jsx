...
const EditOvertime = ({ overtime, onClose }) => {
  ...
  const [rateperhour, setRateperHour] = useState(
    overtime ? overtime.rate_per_hours : "",
  );
  const [numberofhours, setNumberofHours] = useState(
    overtime ? overtime.number_of_hours : "",
  );
  ...
  const handleEditOverTime = async (e) => {
    try {
      ...
      const response = await EditOvertime({
        rate_per_hours: ratePerHourValue,
      ...
      <div className="add-group-modal">
        <form action="" onSubmit={handleEditOverTime}>
          <h3 className="create-group-header">
            Edit Overtime for {overtime.firstname} {overtime.lastname}
          </h3>
