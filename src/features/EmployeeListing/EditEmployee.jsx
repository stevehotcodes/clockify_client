...
const EditEmployee = ({ employee }) => {
  const [positionValue, setPositionValue] = useState(
    employee ? employee.position_description : "",
  );
  const [scheduleDescription, setScheduleDescription] = useState(
    employee ? employee.schedule_description : "",
  );
  ...
  const handleEditSchedule = async (e) => {
    e.preventDefault();
    try {
      const response = await editEmployeesSchedule({
  ...
  const handleEditPosition = async (e) => {
    e.preventDefault();
    try {
      const response = await editEmployeePosition({
      ...
      <div className="add-group-modal">
        <form action="" onSubmit={(e) => handleEditSchedule(e)}>
          <h3 className="create-group-header">
            Edit Position and Schedule for {employee.firstname}{" "}
            {employee.lastname}
          </h3>
