...
const EditcashAdvances = ({ cashadvanceDetails, onClose }) => {
  const [amount, setAmount] = useState(
    cashadvanceDetails ? cashadvanceDetails.amount : "",
  );
  const [user_id, setUserID] = useState(
    cashadvanceDetails ? cashadvanceDetails.user_id[0] : "",
  );
  const [editcashAdvancesforAnEmployee] =
    useEditcashAdvancesforAnEmployeeMutation();
  ...
  const handleEditCashAdvance = async (e) => {
    ...
    try {
      if (amountValue == "" || userIDValue == "") {
        ...
        const response = await editcashAdvancesforAnEmployee({
          amount: amount,
          user_id: user_id,
        }).unwrap();
    ...
    <div className="create-position-container">
      ...
      <div className="add-group-modal">
        <form action="" onSubmit={handleEditCashAdvance}>
          <h3 className="create-group-header">
            Edit Advances for {cashadvanceDetails.firstname}{" "}
            {cashadvanceDetails.lastname}
          </h3>
