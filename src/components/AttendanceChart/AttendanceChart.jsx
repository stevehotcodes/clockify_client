...
const AttendanceChart = ({ reportingData }) => {
  ...
  useEffect(() => {
    if (reportingData && reportingData.length > 0) {
      ...
      const earlyCounts = reportingData.map((item) =>
        item.reporting_state === "Early" ? item.count : 0,
      );
      const lateCounts = reportingData.map((item) =>
        item.reporting_state === "Late" ? item.count : 0,
      );
